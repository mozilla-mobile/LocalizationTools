...
struct ExportTask {
    ...
    private func exportLocales() {
        let command =
            "xcodebuild -exportLocalizations -project \(xcodeProjPath) -localizationPath \(EXPORT_BASE_PATH)"
        let command2 =
            locales
    ...
    private func handleXML(path: String, locale: String, commentOverrides: [String: String]) {
        let url = URL(
            fileURLWithPath: path.appending("/\(locale).xcloc/Localized Contents/\(locale).xliff"))
        let xml = try! XMLDocument(
            contentsOf: url, options: [.nodePreserveWhitespace, .nodeCompactEmptyElement])
        ...
        for case let fileNode as XMLElement in fileNodes {
            ...
            for case let translation as XMLElement in translations {
                ...
                if let comment = translation.attribute(forName: "id")?.stringValue.flatMap({
                    commentOverrides[$0]
                }) {
    ...
    private func copyToL10NRepo(locale: String) {
        let source = URL(
            fileURLWithPath:
                "\(EXPORT_BASE_PATH)/\(locale).xcloc/Localized Contents/\(locale).xliff")
    ...
    func run() {
        exportLocales()
        let commentOverrideURL = URL(fileURLWithPath: xcodeProjPath).deletingLastPathComponent()
            .appendingPathComponent("l10n_comments.txt")
        let commentOverrides: [String: String] =
            (try? String(contentsOf: commentOverrideURL))?
        ...
        locales.forEach { locale in
            ...
            queue.async {
                handleXML(
                    path: EXPORT_BASE_PATH, locale: locale, commentOverrides: commentOverrides)
